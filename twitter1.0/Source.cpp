...
int view_login_page()                   //viewing login page
{
	...
	printf("\n\t Password : ");
	scanf(" %[^\n]", pwd);
	login_flag = login(username, pwd); // return userid
...
void post_tweet(int userid){
	...
	storetweet(userid,tweet);
}

void myTweets(int userid, int tweetCount){

	struct tweet **temp = getMyTweets(userid,&tweetCount);
	if (temp == NULL)
	{
...
void getFollowers(int userid, int followerCount){

	char ** followers = myFollowers(userid, &followerCount);
...
void getFollowing(int userid, int followingCount){

	char **following = myFollowing(userid, &followingCount);
...
void searchUser(){
	...
	int count = 0;
	getUserFollowers(res->userid, &count);
	res->followers = count;
	res->following = getFollowing(res->userid);
	res->my_tweet_count = 0;
	view_users_profile(res);
}

void settings(int userid){
	...
	case 1: printf("\tEnter New Username : ");
		fflush(stdin);
		scanf("%[^\n]s", str);
		if(changeUserName(userid, str) == 0){
...
void viewFeed(int userid){
	...
	struct tweet **feed = NULL; // showFeed(userid, &len); return all tweets
...
void execute_features(struct user* currentUser, int userid, int option){

	switch (option){
	case 1: post_tweet(userid);
		break;
	case 2: viewFeed(userid);
		break;
	case 3: myTweets(userid, currentUser->my_tweet_count);
		break;
	case 4: getFollowers(userid, currentUser->followers);
		break;
	case 5: getFollowing(userid, currentUser->followers);
		break;
	case 6: getTrending();
		break;
	case 7: searchUser();
		break;
	case 8: settings(userid);
		break;
	case 9:updateAll();
		loadData();
		break;
...
int main(void)
{
	...
	switch (choice){
	case 1: userid = view_login_page();
		printf("\n\n\t  Login Successful !!\n\n");
		currentUser = displayUsersProfile(userid);// displays user Informations
		break;
	case 2: view_sign_up_page();
		system("cls");
		userid = view_login_page();
		currentUser = displayUsersProfile(userid);//  displays user Informations
		break;
	case 3: return 0;
	}
	...
	while (option != 9){
		...
		execute_features(currentUser, userid, option);
	}
